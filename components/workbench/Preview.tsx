...
export const Preview = memo(({ sendMessage }: PreviewProps) => {
  ...
  const findMinPortIndex = useCallback(
    (minIndex: number, preview: any, index: number, array: any[]) => {
      return (preview.port ?? Infinity) < (array[minIndex].port ?? Infinity)
        ? index
        : minIndex;
    },
    []
    ...
    const messageHandler = (e: any) => {
      if (e.data?.error) {
        ...
        if (Array.isArray(e.data.error)) {
          setErrorLogs([
            e.data.error[0].stack || e.data.error[0].message || e.data.error[0],
          ]);
        } else {
          setErrorLogs(
            e.data.error.message
              ? [e.data.error.message, e.data.error.stack || ""]
              : e.data.error
          );
        }
      }
  ...
  const startProgressAnimation = useCallback(() => {
    ...
    const progressInterval = setInterval(() => {
      ...
      workbenchStore.updateLoadingProgress(
  ...
  const reloadPreview = () => {
    if (iframeRef.current) {
      ...
      // 添加一次性加载事件监听器
      iframeRef.current.addEventListener("load", handleReloadComplete, {
        once: true,
      });
  ...
  return (
    <div
      ...
      className={`w-full h-full flex flex-col relative ${
        isPreviewOnly ? "fixed inset-0 z-50 bg-white" : ""
      }`}
    >
      {isPortDropdownOpen && (
        <div
        ...
        >
          {activePreview?.isLoading && (
            <div className="absolute top-0 left-0 right-0 z-10">
              <Progress
                value={activePreview.loadingProgress}
          ...
          {activePreview && !workbenchStore.isFirstDeploy.get() && (
            <>
              ...
              <ScreenshotSelector
                ...
                containerRef={
                  iframeRef as unknown as React.RefObject<HTMLElement>
                }
              />
            </>
          )}
              ...
              <div className="flex items-center p-4">
                <div className="flex flex-col gap-1">
                  <div className="text-amber-400 flex items-start">
                    ...
                    <div className="text-white font-medium leading-5 ml-2">
                      Error
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="text-gray-300 text-sm">
                      Oops, it looks like our AI had a bit of a hiccup and broke
                      the app
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2 p-2">
                  ...
                  >
                    Try to fix
                    <kbd className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">
                      F
                    </kbd>
                  </button>
                  ...
                  >
                    Show logs
                    <kbd className="bg-[rgba(255,255,255,0.1)] px-1.5 py-0.5 rounded text-xs">
                      L
                    </kbd>
                  </button>
                </div>
